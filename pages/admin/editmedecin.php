<?php
  ...
  $id =$_GET['id'];
  $req = new Requette();
  $result = $req->selectOne('medecin','id_medecin',$id);
        ...
        if(preg_match('#^(77||78||76||70)[0-9]{7}$#',  $telephone)){
            if(filter_var($email, FILTER_VALIDATE_EMAIL)){
              foreach ($service as $value) {
                $value = intval($value);
                $donnees = ['prenom'=>$prenom, 'nom'=>$nom, 'telephone'=>$telephone, 'specialite'=>$specialite,'email'=>$email, 'mdp'=>$mdp,'id_service'=>$value];
                $add = new Requette();
                $res =  $add->update($donnees,$id,'medecin','id_medecin');
                if($res){
      ...
      <form action="" method="post">
        ...
        <div class="form-group ">
            <?php
                ...
                echo $forms->formInput('Specialite','text','specialite','Entrez le specialite',$result['specialite']);
